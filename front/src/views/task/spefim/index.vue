<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="spefimList" @selection-change="handleSelectionChange">
...
<!--          <ExcelDisplay ref="exceldisplay" v-if="form.dataFile !== null" :file="form.dataFile"></ExcelDisplay>-->
          <FileDisplay ref="exceldisplay" v-if="form.dataFile !== null" :file="form.dataFile"></FileDisplay>
...
<script>
import { listSpefim, getSpefim, delSpefim, addSpefim, updateSpefim } from "@/api/task/spefim";
...
export default {
  ...
  methods: {
    filedownload() {
      this.download("/profile/upload/2024/02/23/result_20240223162341A001.xlsx");
    },
    handleSuccess(res){
      if(res.code === 200){
    ...
    getList() {
      this.loading = true;
      listSpefim(this.queryParams).then(response => {
        this.spefimList = response.rows;
    ...
    handleAdd() {
      ...
      this.state = "add";
      this.width = "500px";
      this.title = "创建SPEFIM挖掘任务";
    },
    handleDetail(row) {
      this.reset();
      const taskId = row.taskId || this.ids
      getSpefim(taskId).then(response => {
        this.form = response.data;
        this.open = true;
        this.state = "detail";
        this.width = "1000px";
        this.title = "查看SPEFIM挖掘任务配置";
      });
    },
    handleResult(row) {
      this.reset();
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.taskId != null) {
            updateSpefim(this.form).then(response => {
            ...
            addSpefim(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除SPEFIM编号为"' + taskIds + '"的数据项？').then(function() {
        return delSpefim(taskIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('task/spefim/export', {
        ...this.queryParams
      }, `spefim_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
