...
object PEFIM {
  ...
  def pefimfindFrequentItemsets(transactions: Array[Itemset], singletons: Array[(String, Int)], minSupport: Int, total:Int): List[FI] = {
    val sortedSingletons = singletons.map(t => t._1)
    val slfi = singletons.map(t => new FI(List(t._1),t._2*1.0/total)).toList
    val slfimap = singletons.map(t=>(t._1,t._2*1.0/total)).toMap
    println("total:"+total)
    if (sortedSingletons.nonEmpty) {
      ...
      val mul_itemset = preList
        .toList
        .flatMap(t => minePatternFragment(t._1, t._2, minSupport,slfimap,total))
      println("mul_itemset")
      mul_itemset.foreach(m=>println(m.itemset+":"+m.support))
      mul_itemset
    }
    else
      List.empty[FI]
