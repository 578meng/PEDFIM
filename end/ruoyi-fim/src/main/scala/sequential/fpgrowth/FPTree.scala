...
class FPTree2(transactions: List[(Itemset, Int)], minSupport: Int, val singletons: mutable.Map[String, Option[FPNode]],preSingletons: Seq[String]) {
  ...
  for (itemset <- transactions) {
    val items = singletons.keys.toList.sortWith((a,b)=>preSingletons.indexOf(a)<preSingletons.indexOf(b))
    val sortedItemset = itemset._1.filter(i => items.contains(i)).sortWith((a, b) => items.indexOf(a) < items.indexOf(b))
    var currentParentNode = rootNode
    for (item <- sortedItemset) {
      // If path exists, increment, if not, create new node
