...
object FIMTest {
  ...
  def assertItemsetsMatch(expectedSets: List[Itemset], result: List[Itemset], className: String): Unit = {
    try {
      ...
      println("expectedSets:"+expectedSets.size)
      println("result:"+result.size)
//      println("expectedSets.intersect(result):"+expectedSets.map(_.sorted).intersect(result.map(_.sorted)).size)
//      assert(expectedSets.map(_.sorted).intersect(result.map(_.sorted)).size == expectedSets.size)
    }
    catch {
